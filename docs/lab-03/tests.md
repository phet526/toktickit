...
| **API-12** | API | AC-07, FR-11, BR-13 | IT Staff หรือ Admin กดเคลมตั๋วที่ยังไม่มีผู้รับผิดชอบ (Claim Ticket) | HTTP 200; บันทึก `assignedStaffId = currentUser.id` ลงฐานข้อมูล | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass |
| **API-13** | API | AC-07, FR-12, BR-13 | IT Staff หรือ Admin โอนตั๋ว (Reassign) ไปให้ IT Staff/Admin คนอื่นที่ Active | HTTP 200; อัปเดต `assignedStaffId` เป็นเจ้าหน้าที่คนใหม่สำเร็จ | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass |
| **API-14** | API | AC-08, FR-13, BR-14 | IT Staff หรือ Admin อัปเดตระดับ `itPriority` | HTTP 200; อัปเดตค่า `itPriority` ใหม่ โดยที่ `requestedPriority` เดิมยังคงอยู่ | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass |
| **API-15** | API | AC-08, FR-14, BR-16 | IT Staff หรือ Admin ปรับสถานะตั๋วตาม State Transition Matrix | HTTP 200; ปรับสถานะตั๋วสำเร็จตามลำดับที่อนุญาต | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass |
| **API-16** | API | BR-10, BR-12 | Requester, IT Staff หรือ Admin โพสต์ Public Comment บนตั๋ว | HTTP 201; บันทึกข้อความพร้อม Author และ Timestamp แบบ Append-only | `server/tests/lab-03/comments-notes.api.test.ts` | Pass |
| **API-17** | API | BR-11, BR-12 | IT Staff หรือ Admin บันทึก Internal Note บนตั๋ว | HTTP 201; บันทึกข้อความภายในลงระบบสำเร็จ | `server/tests/lab-03/comments-notes.api.test.ts` | Pass |
...
| **UI-04** | UI | AC-07, AC-08 | หน้า Staff & Admin Ticket Detail: เคลมตั๋ว, ปรับ IT Priority, และสลับแท็บ Comments/Notes | คอนโทรลตอบสนองถูกต้อง และกล่อง Internal Note แสดงสีเตือนความปลอดภัย | `client/tests/lab-03/StaffTicketDetail.test.tsx` | Pass |
...
- **Server Unit, API & Authorization Tests:** **114 / 114 Passed (100%)**
  ...
  - `tests/lab-03/auth.api.test.ts` (19 tests)
- **Client UI & Component Tests:** **47 / 47 Passed (100%)**
  - `tests/lab-01/App.test.tsx` (2 tests)
  ...
  - `tests/lab-03/ChangePassword.test.tsx` (7 tests)
