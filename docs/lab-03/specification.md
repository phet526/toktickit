...
- **BR-09 (Administrator Core Role & Superuser Privileges):** Administrator มีหน้าที่หลักในการบริหารจัดการบัญชีผู้ใช้งาน (User Management) ควบคู่กับอำนาจระดับ Superuser ในการเข้าถึง Ticket Queue ส่วนกลาง, เคลมหรือโอนย้ายตั๋ว, ปรับ IT Priority, ปรับเปลี่ยนสถานะตั๋วตาม Matrix, รวมถึงการอ่านและเขียน Public Comments และ Internal Notes เพื่อรองรับการกำกับดูแลเชิงปฏิบัติการและกรณีฉุกเฉิน (Emergency Ticket Escalation) ตามที่กำหนดไว้อย่างชัดเจนในตาราง Authorization Matrix (หัวข้อ 7) และข้อสมมติฐานทางเทคนิค (หัวข้อ 13 ข้อ 4) ซึ่งสอดคล้องกับข้อยกเว้นใน Lab 3 Sheet ข้อ 4.3 ("unless the approved authorization matrix explicitly permits it")

### Comments & Notes Rules
- **BR-10 (Public Comments Visibility & Append-Only):** Public Comments สามารถอ่าน (View) ได้โดย Requester เจ้าของตั๋ว, IT Staff, และ Administrator ส่วนการสร้าง/เขียน (Create) สามารถทำได้โดย Requester เจ้าของตั๋ว, IT Staff, และ Administrator ข้อมูลเป็นแบบ Append-only (ห้ามแก้ไขหรือลบ)
- **BR-11 (Internal Notes Visibility & Append-Only):** Internal Notes เป็นข้อมูลความลับเชิงปฏิบัติการ สามารถอ่าน (View) และสร้าง/เขียน (Create) ได้เฉพาะ IT Staff และ Administrator เท่านั้น ข้อมูลเป็นแบบ Append-only และ Requester ต้องไม่สามารถอ่านหรือรับรู้การมีอยู่ของ Internal Notes ได้ (คืนค่า 403 Forbidden)
...
- **BR-13 (Ticket Assignment):** ตั๋วแต่ละใบสามารถมี Ticket Owner ได้ไม่เกิน 1 คน โดยผู้ที่จะเป็น Ticket Owner จะต้องเป็น Active IT Staff หรือ Administrator
- **BR-14 (IT Priority Default & Independence):** เมื่อตั๋วถูกสร้างขึ้น ค่า `itPriority` จะถูกกำหนดค่าเริ่มต้นให้เหมือนกับ `requestedPriority` โดยอัตโนมัติ และหลังจากนั้นจะมีเพียง IT Staff หรือ Administrator เท่านั้นที่สามารถแก้ไขค่า `itPriority` ได้
...
- **BR-16 (Status Transition Governance):** การเปลี่ยนสถานะตั๋วต้องเป็นไปตาม Status Transition Matrix โดย IT Staff หรือ Administrator เป็นผู้มีอำนาจในการเปลี่ยนสถานะอย่างเป็นทางการ Requester ไม่สามารถเปลี่ยนสถานะตั๋วเป็น Resolved หรือ Closed ได้ด้วยตนเอง
